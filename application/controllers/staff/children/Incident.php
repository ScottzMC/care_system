    ...
    class Incident extends CI_Controller{
        ...
        public function view($code){
            ...
            if(!empty($session_role) && $session_role == "Staff"){
                ...
                $data['incident'] = $this->Children_model->display_incident_by_code($code);
                
                $this->load->view('staff/children/incident/view', $data);
            }else{
        ...
        public function detail($id, $code){
            ...
            if(!empty($session_role) && $session_role == "Staff"){
                ...
                $data['incident'] = $this->Children_model->display_incident_by_id($id);
                
                $this->load->view('staff/children/incident/detail', $data);
            }else{
                redirect('staff/account/login');    
            }
        }
        ...
        public function add(){
            ...
            if(isset($btn_submit)){
                ...
                $add = $this->Children_model->insert_incidents($array);
                
                if($add){ ?>
                    <script>
                        alert('Added Successfully');
                        window.location.href="<?php echo site_url('staff/children/incident/view/'.$child_code); ?>";
                    </script>
              <?php }else{ ?>
                       <script>
                            alert('Failed');
        ...
        public function edit($id, $code){
            ...
            if(!empty($session_role) && $session_role == "Staff"){
                ...
                $this->load->view('staff/children/incident/edit', $data);
                ...
                if(isset($btn_submit)){
                    ...
                    $update = $this->Children_model->update_incidents($id, $array);
                    ...
                    ?>
                        <script>
                            ...
                            window.location.href="<?php echo site_url('staff/children/incident/view/'.$code); ?>";
                        </script>
                  <?php }else{ ?>
                           <script>
                                alert('Failed');
        ...
        public function send_mail($id, $code){
          ...
          $subject = "Incident";
          $body = "Please find below attached the Incident document";
         ...
         $atch = base_url('uploads/children/incident/'.$pdf);
        ...
        <script>
            alert("Sent to Mail");
            window.location.href="<?php echo site_url('staff/children/incident/detail/'.$id.'/'.$code); ?>";
        </script> 
 <?php }
        ...
        public function edit_document($id, $code){
            ...
            for($i=0; $i<$cpt1; $i++){
                ...
                $config1 = array(
                    'upload_path'   => "./uploads/children/incident/",
                    //'upload_path'   => "./uploads/../../uploads/community/",
                    'allowed_types' => "pdf|docx|doc",
                    'overwrite'     => TRUE,
                    'max_size'      => "30000",  // Can be set to particular file size
      ...
      <?php }
        ...
        public function delete(){
           ...
           $this->Children_model->delete_incidents($id); 
        }
        
        public function download($id){
          //$file_name= $this->input->get('file_name');

          $query = $this->db->query("SELECT document FROM children_incidents WHERE id = '$id' ")->result();
          foreach($query as $qry){}
            
          //$data = file_get_contents($file_name);
          $file = 'uploads/incident/'.$qry->document;
